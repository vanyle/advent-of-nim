...
proc findLongestPath(r: seq[string], seen: var HashSet[(int, int)], p: (int, int), g: (int, int), vtype: static[bool] = false): int =
    if p == g:
        return 0
    ...
    for i in r.nei(p, vtype):
        noPath = false
        if i notin seen:
            cost = max(cost, 1 + r.findLongestPath(seen, i, g, vtype))
